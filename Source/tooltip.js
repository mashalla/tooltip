...
var AscDialog = new Class({
	...
	show : function() {
		if(!this.isShowing) {
			...
			if(this.options.isModal) {
				this.add_mask();
				// only adds one if it doesn't exist
				this.mask.setStyles({
				...
				case 1:
					...
					switch (align) {
						...
						case 'ws':
							...
							break;
						case 'e':
							// right of target, middle aligned
							top += (tdim.y / 2 - this.popsize.y / 2);
							break;
						case 'es':
...
function isTabbable(elementToCheck) {
	...
	} else if(elementToCheck.nodeName == 'INPUT' || elementToCheck.nodeName == 'A' || elementToCheck.nodeName == 'TEXTAREA' || elementToCheck.nodeName == 'BUTTON') {
		return true;
	} else {
		return false;
	}
